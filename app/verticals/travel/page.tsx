  ...
  return (
    <div className="min-h-screen bg-[#07090F] text-white">
      ...
      <main className="mx-auto w-full max-w-6xl px-5 pb-20 pt-10 sm:px-8">
          ...
          {partnerTrips.map((trip, index) => (
            <Reveal key={trip.partner} delay={index * 0.06}>
              <article className="rounded-2xl border border-white/[0.06]/80 bg-white/[0.04]/80 p-6">
                ...
                <p className="mt-4 text-sm leading-7 text-white/40">{trip.route}</p>
                <button
                  type="button"
                  className="mt-6 rounded-full border border-white/[0.06] bg-white/[0.04]/[0.02] px-4 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-white/40"
                >
                  Add PDF Link
                </button>
              </article>
        ...
        <Reveal delay={0.14}>
          <article className="mt-8 rounded-2xl border border-[#C49A2E66] bg-[linear-gradient(140deg,rgba(196,154,46,0.06),rgba(255,255,255,0.8))] p-7">
