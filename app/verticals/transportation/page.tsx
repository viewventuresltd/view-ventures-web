            ...
            >
              ...
              {routes.map((route) => {
                ...
                return (
                  <g key={route.id}>
                    {/* Glow layer */}
                    <line
                      x1={route.cx} y1={route.cy}
                      x2={casino.cx} y2={casino.cy}
                      stroke={isHov ? "#C49A2E" : "#1456B4"}
                      ...
                      onMouseEnter={(e) => {
                        ...
                        setTooltip({ x: (route.cx + casino.cx) / 2, y: (route.cy + casino.cy) / 2 });
                      }}
                    ...
                    {isHov && (
                      <circle
                        cx={(route.cx + casino.cx) / 2}
                        cy={(route.cy + casino.cy) / 2}
                        r="4"
                        fill="#C49A2E"
                        opacity="0.8"
                      />
              ...
              {routes.map((route) => {
                ...
                return (
                  ...
                  >
                    <circle
                      cx={route.cx} cy={route.cy} r={isHov ? 9 : 6}
                      fill={isHov ? "#C49A2E" : "#1456B4"}
                      opacity={isHov ? 1 : 0.7}
                      style={{ transition: "all 0.2s" }}
                      filter={isHov ? "url(#glow)" : "none"}
                    />
                    <text
                      x={route.cx + 12} y={route.cy + 4}
                      fontSize="11"
                      fill={isHov ? "#C49A2E" : "rgba(255,255,255,0.55)"}
              ...
              {/* Casino node */}
              <g>
                <circle cx={casino.cx} cy={casino.cy} r="10" fill="#C49A2E" opacity="0.9" filter="url(#glow)" />
                <circle cx={casino.cx} cy={casino.cy} r="16" fill="none" stroke="#C49A2E" strokeWidth="1" opacity="0.3" />
                <text
                  x={casino.cx + 18} y={casino.cy - 4}
                  fontSize="10"
                  fill="rgba(196,154,46,0.9)"
                  fontFamily="monospace"
              ...
              {hovered && hoveredRoute && (
                <g>
                  <rect
                    x={Math.min(hoveredRoute.cx, casino.cx) + Math.abs(hoveredRoute.cx - casino.cx) / 2 - 90}
                    y={Math.min(hoveredRoute.cy, casino.cy) + Math.abs(hoveredRoute.cy - casino.cy) / 2 - 42}
                    width="180"
                    height="52"
                    rx="6"
                    fill="#0C1525"
                    stroke="rgba(196,154,46,0.4)"
                    strokeWidth="1"
                  />
                  <text
                    x={Math.min(hoveredRoute.cx, casino.cx) + Math.abs(hoveredRoute.cx - casino.cx) / 2}
                    y={Math.min(hoveredRoute.cy, casino.cy) + Math.abs(hoveredRoute.cy - casino.cy) / 2 - 22}
                    textAnchor="middle"
                    fontSize="10"
                    fill="#C49A2E"
                    fontFamily="monospace"
                    fontWeight="bold"
                  >
                    {hoveredRoute.name} — Reservations
                  </text>
                  <text
                    x={Math.min(hoveredRoute.cx, casino.cx) + Math.abs(hoveredRoute.cx - casino.cx) / 2}
                    y={Math.min(hoveredRoute.cy, casino.cy) + Math.abs(hoveredRoute.cy - casino.cy) / 2 - 6}
                    textAnchor="middle"
                    fontSize="9"
